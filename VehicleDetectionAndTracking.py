...
class VehicleDetectionAndTracking:
    def __init__(self, min_conf=0.6, max_age=2, max_hits=8, front=True, left=False):
        ...
        for tracker in self.tracker_list:
            if tracker.num_hits >= self.min_hits and tracker.num_unmatched <= self.max_age:
                ...
                if self.front:
                    ...
                    if area >= 100000:
                        cv2.putText(image, 'WARNING', (int(dims[1]/2)-120, 50), cv2.FONT_HERSHEY_DUPLEX, 2.0, (0, 0, 255), 2,
                                    cv2.LINE_AA)
                        area_count += 1
