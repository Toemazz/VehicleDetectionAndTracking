...
class VehicleDetector:
    ...
    def __init__(self, kitti=False, min_conf=0.7, info=False):
        ...
        loaded_label_map = label_map.load_label_map(path_to_label_map)
        categories = label_map.convert_label_map_to_categories(label_map=loaded_label_map,
                                                               max_num_classes=num_classes,
                                                               use_display_name=True)

        # Assign an index to each category
        self.category_index = label_map.create_category_index(categories)
